...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupTaskDone(currentItem: TodoListModel) {
        currentItem.id?.let { todoListViewModel.updateDoneForActivityTask(it, isDone = 1) }
    }
    ...
    private fun subscribeObserve() {
        todoListViewModel.getAllTaskListForActivityResponse.observe(this) { response ->
            when (response) {
                is Resource.Loading -> {
                ...
                is Resource.Success -> {
                    ...
                    todoListViewModel.clearGetAllTaskListForActivityResponse()
                }
            }
        }

        todoListViewModel.updateDoneTaskForActivityResponse.observe(this) { response ->
            when (response) {
                is Resource.Loading -> {
                ...
                is Resource.Success -> {
                    ...
                    todoListViewModel.clearUpdateDoneTaskForActivityResponse()
                }
            }
        }
    }
    ...
    private fun setupGetAllTaskList() {
        todoListViewModel.getAllTaskListForActivity()
    }
    ...
    private fun setupOverlay() {
        ...
        mView?.let { view ->
            view.apply {
                ...
                tvCounter.text = "$countValue ${resources.getString(R.string.active)}"
